...
\begin{itemize}
  ...
  \item Specify the sign $s$ explicitly via \texttt{--chirality-sign} (\texttt{positive} $= +1$, \texttt{negative} $= -1$), determined from an on-site observation of the as-built array:
  \[
    s = \begin{cases} +1 & \text{if $c$ is to the \emph{left} of the reference bearing line} \\
                      -1 & \text{if $c$ is to the \emph{right}} \end{cases}
  \]
  ...
  \item Zero cost when the sign matches the constraint; the penalty only fires if the optimizer tries to cross into the mirror solution
  \item CLI: \texttt{--chirality-index}~$c$ + \texttt{--chirality-sign} \{\texttt{positive},\texttt{negative}\} (requires \texttt{--rot-index})
...
\begin{itemize}
  ...
  \item \texttt{--chirality-index} + \texttt{--chirality-sign}: fix reflection (handedness)
