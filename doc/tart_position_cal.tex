...
\affil[*]{Corresponding Author: tim@elec.ac.nz}
...
In an interferometric array, an arbitrary point is chosen as the {\em phase-centre} of the array. This phase-centre is the (0,0) point for the antenna coordinate frame. The purpose of this calibration is to estimate accurately the positions of each antenna with respect to this phase centre.
...
This is accomplished by identifying an antenna that has a radial vector that aligns with a distant, identifiable landmark visible on satellite imagery (e.g.\ Google Maps). The geographic coordinates (latitude and longitude) of both the phase-centre and the landmark are recorded.

From these two geographic coordinates, the expected bearing $\theta_k$ of the reference antenna $k$ is computed.
This $\theta_k$ measurement is supplied to the least-squares procedure as $L_\theta$ in Equation~4, described below. In the software described in~\ref{sec:software} $\theta_k$ is computed as an inverse geodesic problem implemented in the PROJ library~\cite{proj}.

Given any set $\{x_i, y_i\}$ of 24 antenna positions we can simulate all the measurements, $\theta_k'$, $\delta_{ij}'$ and $\theta_k'$ that we would expect.
\begin{align}
	r_i'         & =  \sqrt{x_i^2 + y_i^2}                \\
	\delta_{ij}' & = \sqrt{(x_i - x_j)^2 + (y_i - y_j)^2} \\
	\theta_k'    & = \arctan2\!\left(x_k, y_k\right)
\end{align}

These simulated measurements will be close to the recorded measurements if the $\{x_i, y_i\}$ are close to the true values. The differences between the simulated measurements and the actual measurements are called {\em residuals}. The set of positions that minimizes the squared residuals $L(\{x_i, y_i\})$, is a good estimate of the true antenna positions $\{x_i, y_i\}$.
\begin{align}
	L(x, y)       & = L_r(x,y) + L_{ij}(x,y) + \lambda L_\theta(x,y)                \\
	L_r(x,y)      & = \sum_{i=1}^{N} \left( r_i - r_i' \right)^2                    \\
	L_{ij}(x,y)   & = \sum_{i,j=1}^{N} \left( \delta_{ij} -  \delta_{ij}' \right)^2 \\
	L_\theta(x,y) & = \left( \theta_k - \theta_k' \right)^2
\end{align}
In practice, the angular residual term is also weighted by a factor of $\lambda$ relative to the pairwise term, reflecting the higher confidence in the radial measurements.
