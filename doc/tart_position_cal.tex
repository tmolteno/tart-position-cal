...
% TKTK in the coordinate system of the phase-centre of the antenna array. Talk about this 
% when discussing radial measurements.
...
This $\theta_k$ measurement is supplied to the least-squares procedure (as $L_\theta$ in Equation~4).
described below. In the software described in~\ref{sec:software} $\theta_k$ is computed as an inverse geodesic problem implemented in the PROJ library~\cite{proj}.
...
\begin{align}
 ...
 \theta_k' &= \arctan\!\left(\frac{x_k}{y_k}\right)
\end{align}
...
\section{The tart-position-cal python package}
\label{sec:software}
...
Applying IRLS with Tukey weights to the UNAM data reduced the inter-antenna residual statistics: median absolute deviation from 0.57~mm to 0.51~mm, standard deviation from 1.07~mm to 1.10~mm, 50th percentile from 0.57~mm to 0.51~mm, and 90th percentile from 1.53~mm to 1.50~mm. The outer loop converged in 10 iterations. Figure~\ref{fig:irls_residuals} shows the residual histogram for the IRLS calibration. Compared to the manual iterative re-measurement workflow, IRLS provides a more robust solution, though manual verification of flagged outlier pairs is still required, fewer measurement need to be redone.
